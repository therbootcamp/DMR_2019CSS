...
2. `theme`s anpasst.
3. `scale`s anpasst.
4. Bilddateien erstellt.
5. Multiple Plots zusammenstellt.
...
1. Verwende die `read_csv()` Funktion um den Datensatz `krimi` als Objekt `krimi` einzulesen. Denke an den Trick mit den Anführungszeichen. 
...
2. Sieht noch nicht informativ aus. Ergänze `scale_x_continuous(trans = 'pseudo_log')` und `scale_y_continuous(trans = 'pseudo_log')` um die y und x-Achse zu stauchen. Später noch mehr zu Skalierungen. 
...
3. Besser. Jetzt zeichnet sich ein schwach positiver Zusammenhang ab. Könnte das anders aussehen, wenn man die verschiedenen Klassen von Verbrechen differenziert? Versuche dies mal indem du `verbrechen` dem `col` Argument in `aes()` zuweist.
...
4. Nicht wirklich informativ, oder? Anstatt mit `col` versuche nun die verschiedenen Verbrechen mit `facet_wrap()` zu differenzieren.
...
7. Der Zusammenhang ist verschwunden. Wir haben von Beginn an übersehen, dass viele Metro-fahrer auch eine hohe Bevölkerungsdichte bedeutet und diese ist mit der Anzahl von Delikten assoziiert. Spiele ein wenig herum. Welche Zusammenhänge kannst du noch entdecken?      

### D - `theme`s

In diesem Abschnitt passt du deinen Lieblingsplot aus dem letzten Abschnitt mit der `theme()` Funktion an. 

1. Zuallererst speichere deinen Plot als ein `gg` Objekt mit name `verbrechen_gg`.
...
4. Ändere nun die Farbe des Hintergunds der Panel-Überschriften zu `white` mit dem `strip.background` Argument und wiederum der `element_rect()` Helferfunktion.
...
2. Als erstes verwende `sclale_size()` und darin das `range` argument um die Grösse der Punkte zu verkleinern. Damit kannst du die Überlappung etwas veringern. Probiere ein paar Werte für die untere und obere Grenze aus, die jeweils kleiner als `10` sein sollten, um einen guten Trade-off zwischen Grösse und Überlappung zu finden.
...
5. Alternativ kann man die Farbskala selbst setzen. Verwende `scale_color_manual()` und die `viridis()` Funktion aus dem `viridis` Paket (nicht vergessen zu laden) um die Farben manuell anzupassen. Viridis ist ein anderer etablierter, etwas schönerer Farbsatz, der ebenfalls Helligkeitsunterschiede berücksichtigt.   
...
1. Es ist an der Zeit euren Plot als eine Bilddatei zu speichern. Dafür musst du zuerst einen neuen Unterordner in einem Projektordner mit dem Namen `3_Figures` anlegen, sofern dieser noch nicht existiert (sollte bereits vorhanden sein).
