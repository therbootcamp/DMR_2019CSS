...
In diesem Practical wirst du üben Daten mit `dplyr` zu transformieren. 
...
1. Verwende die `read_csv()` Funktion um den Datensatz `appointments` als Objekt `appointments` einzulesen. Denkt an den Trick mit den Anführungszeichen. 
...
### C - Change column names with `rename()`
...
2. Eine der Spalten hat aktuell den namen `Age`. Verwende `rename()` um den Namen in `alter_jahre` zu ändern um klar zu machen, dass es sich um das Alter in Jahren handelt.   

```{r, echo = TRUE, eval = FALSE}
# Änder Age zu alter_Jahre
...
2. Verwende `mutate()` um eine Spalte `krankheitsgrad` zu ergänzen, die anzeigt wie krank der Patient ist. Addiere hierzu die Spalten `bluthochdruck`, `diabetes`, `alkoholismus`. Erinnere im Hintergrund: `TRUE = 1` und `FALSE = 0`. 
...
4. Verwende `mutate()` und `case_when()` um `M` und `F` in der Spalte `geschlecht` in `männlich` respektive `weiblich` zu verändern. Weise hierzu die entstehende Variable direkt wieder der Spalte `geschlecht` zu.
...
5. Verwende `mutate()` und `case_when()` um `No` und `Yes` in der Spalte `termin_eingehalten` in `0` respektive `1` zu verändern. Kreiere eine neue Spalte mit Namen `termin_eingehalten_01`.  
...
```
6. Finde raus mit der bekannten Form `data_frame$spalte1[data_frame$spalte2 == XX]` und der `mean()` Funktion, ob schwerkranke Patienten häufiger den Termin nicht wahrnehmen (verwende `termin_eingehalten_01`).  
...
7. Vergleiche nun, ob es vielleicht Unterschiede zwischen Männern und Frauen gibt und zwischen Patienten, die mehr oder weniger als 360 Monate alt sind.  
...
2. Printe `appointments_reduziert`. Sind die richten Spalten enthalten?
...
4. Printe `appointments_reduziert$geschlecht`. Noch Männer enthalten?
...
6. Printe `appointments_reduziert$alter_jahre`. Noch Jüngere enthalten?

7. Verwende `arrange()` um den `appointments_reduziert` aufsteigend nach Alter zu ordnen. Ergänze die Pipeline. 
...
8. Printe `appointments_reduziert` und `appointments_reduziert$alter_jahre`. Alles schön sortiert?
...
10. Printe `appointments_reduziert` und `appointments_reduziert$alter_jahre`. Alles schön sortiert?

11. Verwende `arrange()` und `desc()` um `appointments_reduziert` auch absteigend (descending) nach `krankheitsgrad` zu ordnen. 
...
12. Printe `appointments_reduziert` und `appointments_reduziert$krankheitsgrad`. Alles schön sortiert?

13. Ändere die Reihenfolge von `krankheitsgrad` und  `alter_jahre` um primär nach `krankheitsgrad` zu sortieren. 
...
14. Verwende `slice()` um die ersten zwanzig Fälle in `appointments_reduziert`. Ergänze die Pipeline.
...
1. Verwende die `read_csv()` Funktion um den Datensatz `weather`, welcher das Wetter in Brasilien enthält, als Objekt `weather` einzulesen. Denkt an den Trick mit den Anführungszeichen. 
...
4. Verwendet `select()` und `rename()` um die Spalten `YYYYMMDD` und `PRECTOT` auszuwählen und sie in `termin_tag` und `niederschlag` umzubenennnen. Nennt das resultierende Objekt `weather_reduziert`
...
5. Printe `weather_reduziert`. Alles wie gewünscht?

6. Verwendet `left_join()` um `weather_reduziert` mit den Spalten `termin_tag` und `termin_eingehalten` in `appointments` zu verbinden. Nennt den gemeinsamen Datensatz `appointments_weather`. 
...
7. Printe `appointments_weather`. Alles in Ordnung?

8. Verwendet `select()` um nach dem verbinden die Spalten in die folgende Reihenfolge zu bringen: `termin_tag`, `niederschlag`, und `termin_eingehalten`. 
...
9. Printe `appointments_weather`. Alles in Ordnung?

10. Erstellt im nächsten Schritt der pipeline erneut eine Variable `termin_eingelten_01` wie bereits oben. Ausserdem erstellt eine Variable `niederschlag_01` die anzeigt, ob es Niederschlag gab oder nicht. 
...
11. Printe `appointments_weather`. Alles in Ordnung?

12. Finde raus nun mit der bekannten Form `data_frame$spalte1[data_frame$spalte2 == XX]` und der `mean()` Funktion heraus, ob Termine bei Regen seltener eingehalten wurden.  
...
1. Versuche alle bisherigen Schritte beginnend beim einlesen bis zum Verbinden der Datensätze in eine einzige Pipeline zu bringen. Das zielt ist es die letzte Version von `appointments_weather` zu erstellen ohne auch nur einmal ein Objekt zu kreieren. 
...
Siehe [https://cran.r-project.org/web/packages/dplyr/vignettes/dplyr.html](https://cran.r-project.org/web/packages/dplyr/vignettes/dplyr.html) für die `dplyr` vignette.
