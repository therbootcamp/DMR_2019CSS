...
1. Verwende den Code unten um einen Violinen Plot der `Calories` als Funktion der `Category` zu kreieren. Violinenplots zeigen mit ihrer Dicke die jeweilige relative Häufigkeit der Werte, in diesem Fall der Kalorien. 
...
1. Mit der `stat_summary` können auch direkt einfache Statistiken berechnet und geplotted werden. Verwende den folgenden Code um die `Calories` (y-Achse) gegen `Categories` zu plotten. Annotiere angemessen. 
